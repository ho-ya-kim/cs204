...
> 자연수 n에 대해, k번 검사한다고 가정하자.
>
> n-1을 2^s * d 형태 (d는 홀수)로 바꾸고, 아래를 k번 반복한다.
>
> 임의의 a ∈ {2, 3, ..., n-1}, 모든 r ∈ {0, 1, ..., s-1}에 대해 다음을 검사한다.
>
> <div align="center">
> a^d mod n ≠ 1 & a^(2^r * d) mod n ≠ n-1
> </div>
>
> * 위를 만족하면 합성수
...
16. **Isomorphic Graphs**: 동형 그래프
    - 두 그래프 G1, G2에 대해 G1의 정점들과 G2의 정점들 사이에 전단사 함수 f가 존재하여, G1의 임의의 두 정점 u,v가 연결되어 있다�� f(u),f(v)도 연결되어 있고 그 역도 성립하는 경우를 말함.
...
2. **n개의 정점을 가진 트리는 n-1개의 간선을 가진다**
    ...
    - k+1개 정점을 가진 트리 T에서:
    - 리프 노드 하나�� 그에 연결된 간선을 제거하면 k개 정점을 가진 트리 T'
